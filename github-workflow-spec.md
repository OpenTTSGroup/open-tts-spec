    ...
    steps:
      ...
      - name: Ensure submodules are populated
        run: |
          ...
          # 目录固定为 engine(见 project-layout-spec §4),按上游核心文件验证
          # ls engine/pyproject.toml  # 或 engine/requirements.txt / engine/setup.py
...
- name: Ensure submodules are populated
  run: |
    ...
    ls engine/pyproject.toml  # 或其他上游核心文件(requirements.txt / setup.py 等)
```

规范固定 submodule 目录为 `engine`(见 `project-layout-spec.md` §4),直接使用即可;`ls` 任一关键文件用于失败即早,不需要遍历全部。
